...
| 2026-06-12 | Antigravity | 広告バナー表示/非表示（ダミー可視化）、ステージ順次解放（ロックUI/機能）、 Next Stage 遷移時のインタースティシャル広告確率発生（全画面ダミー広告シミュレータ）、およびデバッグメニュー（強制クリア・全解放・セーブ初期化）の実装 |
| 2026-06-06 | Antigravity | パレット空マスタップ時の選択キャンセル（要望①）、およびタップ座標からの近傍吸い寄せ（マグネティックタップ）補正ロジック（要望②）の実装 |
...
Assets/Scripts/Ads/
└── AdManager.cs        ✅ Google AdMob スタブ実装（SDK未導入でもビルド可能・ダミー広告表示）
├── DebugMenu.cs        ✅ デバッグパネル（強制クリア・全アンロック・進行リセット）
...
- TASK-012: AdMob SDK バナー/インタースティシャル表示制御およびエディタ用ダミーUIの実装
- TASK-014: ステージ進行制限・順次解放仕様（🔒表示、非アクティブ化、プレビュー暗転）の実装
- TASK-015: デバッグメニュー（右上のボタン、全アンロック、進行リセット、強制クリア）の実装
