...
セル1つの見た目（`GemCellView.cs`、2026-07-11実装確認）:
- 背景（`_backgroundImage`）＝フルサイズ。**`goalColor` を持つマスは正解・不一致・空のどの状態でも常に `GetColor(goalColor)`（目標色そのまま鮮やか）で、状態によって変化しない**（置くべき色のヒントを常に表示し続けるための仕様。過去のドキュメントに「不一致なら薄く表示・空はグレー」という誤記があったが、実装はそうなっていない）。`goalColor` を持たないマス（自由マス）のみ、空きなら`_emptyColor`（グレー）、ジェムがあれば固定のダーク背景になる。
- ジェム本体（`_gemImage`）＝角丸スプライト、背景の72%スケールで中央配置。正解時は影(Shadow)を消してフラットに見せる（背景と同色になり、ソケットに収まったように見える）。
- ソケット（`_socketImage`）＝ジェムが無い（空マス）の時だけ表示するくぼみ演出。ジェムが置かれると（正解でも不一致でも）非表示になる。
